...
def build_rate_sheet(sheet_title, product_code, display_name, extra_note=""):
    ...
    # Plan-level riders (rate_per=1): the rate IS the full annual premium.
    is_plan_level = prod["rate_per"] == 1

    write_title(ws, f"{display_name} ({product_code})", n_cols)
    if is_plan_level:
        note_parts = ["เบี้ย = เต็มราคาต่อปี (rate = full annual premium ต่อแผน)"]
    else:
    ...
    rate_header = (
        "เบี้ยต่อปี (บาท)" if is_plan_level
        else f"อัตราเบี้ย (บาท/{prod['rate_per']:,} ทุน)"
    )
    example_header = "เบี้ย/ปี (occ 1-2)" if is_plan_level else "เบี้ยตัวอย่าง ทุน 1M"
    write_headers(ws, 4, [
        "แผน", "อายุต่ำสุด", "อายุสูงสุด", "เพศ",
        rate_header, "เฉพาะต่ออายุ?", example_header,
    ])
    ...
    for i, r in enumerate(rs):
        ...
        for j, v in enumerate(vals, 1):
            ...
            if j == 5:
                c.alignment = RIGHT
                c.number_format = "#,##0" if is_plan_level else "#,##0.00"
        # Example premium formula:
        #   plan-level: rate (as-is — no scaling)
        #   sum-scaled: rate × (1M / rate_per)
        if is_plan_level:
            ex = ws.cell(row=rr, column=7, value=f"=E{rr}")
        else:
...
flat_codes = ["MSI1808", "MQR1206"]  # MDP moved to own sheet (now has age-banded plans)
row_ix = 5
...
build_rate_sheet("Rider HBP", "HBP", "ค่ารักษาพยาบาลรายวันพิเศษ (HBP)",
...
build_rate_sheet("Rider HSMHPDC ดับเบิลฯ", "HSMHPDC", "ปลดล็อค ดับเบิล แคร์ (IPD)",
...
build_rate_sheet("Rider OPDMFCPN ALL", "OPDMFCPN_ALL", "เฟิร์สคลาส อัลตร้า แพลทินัม OPD (ทุกรพ.)",
...
build_rate_sheet("Rider HSMFCBN ALL", "HSMFCBN_ALL", "เฟิร์สคลาส อัลตร้า บียอนด์ แพลทินัม IPD (ทุกรพ.)",
                 extra_note="แผนเดียว · Beyond Platinum general · เบี้ยต่ำกว่าเวอร์ชัน BDMS ~15% · อายุรับ 11–70 · ต่ออายุถึง 98")
...
build_rate_sheet("SLA85 A85X", "SLA85", "อยุธยาชีวิตมั่นคง A85/X",
                 extra_note="4 แผน (10/15/20/25 ปี) คุ้มครองถึง 85 · เบี้ยคงที่ตามอายุเริ่ม · A85/25 ถึงอายุ 60 เท่านั้น")
build_rate_sheet("MDP ดับเบิล พลัส", "MDP", "มาย ดับเบิล พลัส (มีเงินปันผล)",
                 extra_note="4 แผน · 15/6 flat 230 ทุกอายุ · 18/10 ages 0-65 · 22/15 & 25/20 ages 0-60")
