  ...
  return (
    <>
      ...
      {showHint && hintContent && (
        <div className="bg-blue-50 border border-blue-100 rounded-lg p-2.5 text-[14px] text-blue-900 leading-relaxed space-y-1.5">
          {hintContent}
        </div>
      )}
    </>
  ...
  return (
    <>
      ...
      {kind === "recurring" && onUpdateStartAge && onUpdateEndAge && (
        <div className="flex items-center gap-2 text-[14px] text-gray-600 flex-wrap">
          <span>อายุ</span>
          <AgeInput
            value={startAge}
...
function KindChip({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex-1 px-3 py-1 rounded-lg text-[14px] font-bold transition ${
        active
...
function InflationInput({
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <span className="text-[13px] text-gray-400">เงินเฟ้อ:</span>
      ...
      <div className="flex items-center gap-1 flex-wrap">
        {INFLATION_HINTS.map((h) => (
          <button
            ...
            className={`px-2 py-0.5 rounded-full text-[13px] transition ${
    ...
    <>
      ...
      {hasData && canExpand && (
        <button
          ...
          className={`w-full flex items-center justify-center gap-1 text-[13px] ${accent.chip} font-bold hover:underline pt-1`}
        >
          {showTable ? (
            <>
...
function SubCalcCard({
  ...
  return (
    <>
      ...
      <Link
        href={subCalcHref}
        className={`flex items-center justify-between ${accent.button} text-white rounded-lg px-3 py-2 text-[14px] font-bold hover:opacity-90 transition`}
      >
      ...
      {hasData ? (
        ...
        <div className="text-[13px] text-gray-400">ยังไม่มีรายการ</div>
      )}
    </>
  );
}
