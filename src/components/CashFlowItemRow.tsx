    ...
    >
            ...
            <div className="flex items-center gap-1 mt-0.5 flex-wrap">
              {onCategoryClick && (
                <button
                  ...
                  className={`flex items-center gap-1 text-[13px] px-1.5 py-0.5 rounded transition ${
                    recurringType === "income"
              ...
              {secondaryLabel && onSecondaryLabelClick && (
                <button
                  ...
                  className="flex items-center gap-1 text-[13px] px-1.5 py-0.5 rounded bg-amber-100 text-amber-700 hover:bg-amber-200 transition"
                >
                  <Tag size={9} />
                  {secondaryLabel}
                </button>
      ...
      {showEssentialToggle && (amount > 0 || isEssential) && (
        <div className="flex items-center gap-2 px-3 pb-2 pt-0">
          ...
          <span className={`text-[14px] ${isEssential ? "text-red-600 font-medium" : "text-gray-400"}`}>
      ...
      {salaryPercent !== undefined && onSalaryPercentChange && (
        <div className="flex items-center gap-1.5 px-3 pb-2 pt-0 flex-wrap">
          <span className="text-[13px] text-gray-500 mr-1">{percentLabel || "หัก % เงินเดือน:"}</span>
          {(percentOptions || [2, 3, 4, 5, 6, 7, 8, 9, 10, 15]).map((pct) => (
            <button
              key={pct}
              onClick={() => onSalaryPercentChange(salaryPercent === pct ? 0 : pct)}
              className={`text-[14px] px-1.5 py-0.5 rounded-full transition-all ${
                salaryPercent === pct
