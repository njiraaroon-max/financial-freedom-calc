            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
                {gauges.map((g, i) => (
                  ...
                  >
                    <div className="text-[9px] md:text-[10px] font-bold text-gray-600 text-center mb-1">{g.desc}</div>
                    <GaugeChart
                      value={g.gaugeVal}
                      displayValue={g.val}
                  ...
                  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => setSelectedGauge(null)}>
                    <div className="bg-white rounded-2xl shadow-xl p-5 mx-4 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
                      ...
                      <div className="bg-gray-50 rounded-xl p-3 mb-4">
                        <div className="text-[10px] font-bold text-gray-500 mb-1">📊 สูตรคำนวณ</div>
                        <div className="text-[11px] text-gray-600 whitespace-pre-line font-mono">{g.formula}</div>
                      </div>
        ...
        <div className="md:grid md:grid-cols-2 md:gap-4 space-y-4 md:space-y-0">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4">
            ...
            <div className="grid grid-cols-3 gap-2 text-center mb-3">
              <div className="bg-emerald-50 rounded-xl p-2">
                <div className="text-[9px] text-gray-500">รายรับ</div>
                <div className="text-xs font-bold text-emerald-700">฿{fmtM(annualIncome)}</div>
              </div>
              <div className="bg-red-50 rounded-xl p-2">
                <div className="text-[9px] text-gray-500">รายจ่าย</div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4">
            ...
            <div className={`rounded-xl p-3 text-white mb-3 ${netWorth >= 0 ? "bg-gradient-to-r from-emerald-500 to-teal-600" : "bg-gradient-to-r from-red-500 to-rose-600"}`}>
              <div className="text-[10px] opacity-80">ความมั่งคั่งสุทธิ (Net Worth)</div>
              <div className="text-xl font-extrabold">฿{fmt(netWorth)}</div>
            </div>

            <div className="grid grid-cols-2 gap-2">
              <div className="bg-emerald-50 rounded-xl p-2.5">
                <div className="text-[9px] text-gray-500">สินทรัพย์รวม</div>
                <div className="text-xs font-bold text-emerald-700">฿{fmt(totalAssets)}</div>
              </div>
              <div className="bg-red-50 rounded-xl p-2.5">
                <div className="text-[9px] text-gray-500">หนี้สินรวม</div>
        ...
        <div className="md:grid md:grid-cols-2 md:gap-4 space-y-4 md:space-y-0">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4">
            ...
            <div className={`rounded-xl p-3 text-white text-center ${efMonths >= 6 ? "bg-gradient-to-r from-emerald-500 to-teal-600" : efMonths >= 3 ? "bg-gradient-to-r from-amber-500 to-orange-500" : "bg-gradient-to-r from-red-500 to-rose-600"}`}>
              <div className="text-[10px] opacity-80">สำรองได้</div>
              <div className="text-2xl font-extrabold">{efMonths.toFixed(1)} เดือน</div>
              <div className="text-[10px] opacity-70">สภาพคล่อง ฿{fmt(liquidAssets)} / รายจ่ายจำเป็น ฿{fmt(monthlyEssential)}/เดือน</div>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4">
            ...
            <div className="flex items-center mb-3">
              <div className="flex flex-col items-center" style={{ width: 44, flexShrink: 0 }}>
                ...
                <span className="text-[8px] text-gray-500 mt-0.5">ปัจจุบัน</span>
              </div>
              <div style={{ flex: yearsToRetire }} className="relative mx-1">
                <div className="border-t-2 border-dashed border-[#1e3a5f]" />
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 text-[9px] font-bold text-[#1e3a5f] bg-white px-1">{yearsToRetire} ปี</div>
              </div>
              <div className="flex flex-col items-center" style={{ width: 44, flexShrink: 0 }}>
                <img src="/icons/retired.png" alt="เกษียณ" width={28} height={28} />
                <span className="text-[8px] text-[#1e3a5f] font-bold mt-0.5">เกษียณ</span>
              </div>
              <div style={{ flex: yearsAfterRetire }} className="relative mx-1">
                <div className="border-t-2 border-dashed border-gray-400" />
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 text-[9px] font-bold text-gray-500 bg-white px-1">{yearsAfterRetire} ปี</div>
              </div>
              <div className="flex flex-col items-center" style={{ width: 44, flexShrink: 0 }}>
                <img src="/icons/bed.svg" alt="อายุขัย" width={28} height={28} className="opacity-50" />
                <span className="text-[8px] text-gray-500 mt-0.5">อายุขัย</span>
              </div>
            </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-4">
          ...
          <div className="flex items-center justify-center gap-2 mb-2">
            <div className="w-[80px] h-[80px] md:w-[90px] md:h-[90px] rounded-full bg-[#1e3a5f] flex flex-col items-center justify-center text-white shrink-0">
              <div className="text-[7px] opacity-80">ค่าใช้จ่าย</div>
              <div className="text-[9px] font-extrabold text-amber-300">พื้นฐาน</div>
              <div className="text-[8px] font-bold mt-0.5">฿{fmtM(basicRetireFund)}</div>
            </div>
            ...
            <div className="w-[80px] h-[80px] md:w-[90px] md:h-[90px] rounded-full bg-[#8b2020] flex flex-col items-center justify-center text-white shrink-0">
              <div className="text-[7px] opacity-80">ค่าใช้จ่าย</div>
              <div className="text-[9px] font-extrabold text-amber-300">พิเศษ</div>
              <div className="text-[8px] font-bold mt-0.5">฿{fmtM(totalSpecialFV)}</div>
            </div>
            <span className="text-gray-400 font-bold text-sm">=</span>
            <div className="w-[80px] h-[80px] md:w-[90px] md:h-[90px] rounded-full bg-amber-400 flex flex-col items-center justify-center shrink-0">
              <div className="text-[7px] text-[#1e3a5f] font-bold">ทุนเกษียณ</div>
              <div className="text-[9px] font-extrabold text-[#1e3a5f]">฿{fmtM(totalRetireFund)}</div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center gap-3 mb-3">
            <div className="bg-amber-400 rounded-lg px-3 py-2 text-center">
              <div className="text-[8px] text-[#1e3a5f] font-bold">ทุนเกษียณ</div>
              <div className="text-[10px] font-extrabold text-[#1e3a5f]">฿{fmtM(totalRetireFund)}</div>
            </div>
            ...
            <div className="w-[70px] h-[70px] rounded-full bg-emerald-500 flex flex-col items-center justify-center text-white">
              <div className="text-[7px] opacity-80">แหล่งเงินทุน</div>
              <div className="text-[8px] font-bold">฿{fmtM(totalSavingFund)}</div>
            </div>
            <span className="text-gray-400 font-bold">=</span>
            <div className={`rounded-xl px-3 py-2 text-center text-white ${shortage <= 0 ? "bg-emerald-600" : "bg-red-500"}`}>
              <div className="text-[8px] opacity-80">{shortage <= 0 ? "เพียงพอ!" : "ต้องเตรียมเพิ่ม"}</div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4">
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              <div className="bg-orange-50 rounded-xl p-2.5">
                <div className="text-[9px] text-gray-500">Bad Case</div>
              ...
              <div className="bg-indigo-50 rounded-xl p-2.5 ring-1 ring-indigo-200">
                <div className="text-[9px] text-gray-500">Base Case</div>
                <div className="text-sm font-extrabold text-indigo-700">฿{fmtM(investAtRetire)}</div>
              </div>
              <div className="bg-emerald-50 rounded-xl p-2.5">
                <div className="text-[9px] text-gray-500">Good Case</div>
